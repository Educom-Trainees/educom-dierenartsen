<template>
  <table
    ...
    class="table table-bordered table-responsive h-100"
  >
    <tr>
      <th id="th-clock">
      ...
      <th id="th-doc">
        ...
        <span id="nr-appointment" class="me-2">{{
          appointments.filter((a) => a.doctor == doctorId || a.doctor == 3)
            .length
        }}</span>
      </th>
    </tr>
    <tr
      v-for="(timeslot, index) in calculatedTimeslots"
      :data-id="timeslot.id"
      :key="timeslot.id"
    >
      ...
      <td
        ...
        class="has-event"
        :rowspan="timeslot.appointment.duration / 15"
      >
        <OverviewAppointment
...
export default {
  ...
  methods: {
    async sortTimeslotsPerDoctor() {
      try {
        ...
        this.timeslotsDoctor = allTimeSlots.filter(
          (timeslot) => timeslot.doctor == this.doctorId
        );
      } catch (error) {
    ...
    async dragAndDropAppointment(event) {
      ...
      const newStartTime = event.target
        .closest("tr")
        .getElementsByClassName("timeslot")[0].innerHTML;
  ...
  computed: {
    calculatedTimeslots() {
      const app = this.appointments.filter(
        (a) => a.doctor == this.doctorId || a.doctor == 3
      );
