  ...
  >
    ...
    <tr v-for="item in calculatedTimeslots">
      ...
      <td
        ...
        class="has-event"
        :rowspan="item.appointment.duration / 15"
      >
        <div class="dropdown">
          <button
            ...
            aria-expanded="false"
            :style="{
              height: 25 * (item.appointment.duration / 15) + 'px',
              'background-color': color + '!important',
            }"
          >
            <span class="appointment-info"
              >{{ item.appointment.customer }} &mdash;
...
export default {
  ...
  data() {
    return {
      tslot: TIMESLOTS.map((t) => {
        return { time: t, doctor: this.doctorId };
      }),
