        ...
        <div class="row">
            ...
            <div class="col-12 col-lg-9 ml-2">
                <div class="chooseDate choose-group">
                    ...
                    <div class="d-flex justify-content-between choose">
                        <div class="d-flex justify-content-around dates-box">
                            ...
                            <div v-for="day in upcomingDays" @click="changeDate(day)"
                ...
                <div class="chooseDoctor choose-group">
                    ...
                    <div class="choose">
                        <button @click="changeDoctorPreference(0)" type="button" :class="{select: preference == 0}">Geen voorkeur</button>
                        <button @click="changeDoctorPreference(1)" type="button" :class="{select: preference == 1}">Karel Lant</button>
                        <button @click="changeDoctorPreference(2)" type="button" :class="{select: preference == 2}">Danique de Beer</button>
                    </div>
                </div>
                <!---->
                <div class="chooseTime choose-group">
                    ...
                    <div class="choose">
                        <div class="w-70 d-flex flex-wrap">
                            <button v-for="timeslot in freeTimeslots.filter(t => t.time < '14:00')" 
                                :class="{select: time == timeslot.time}" 
                                @click="changeTimeslot(timeslot.time, timeslot.doctor)">
                                <img style="width: 20%;" src="../assets/time.png">
                                {{ timeslot.time }}
                            </button> 
                        </div>
                    </div>
                    ...
                    <div class="choose">
                        <div class="w-70 d-flex flex-wrap">
                            <button v-for="timeslot in freeTimeslots.filter(t => t.time >= '14:00')" 
                                :class="{select: time == timeslot.time}" 
                                @click="changeTimeslot(timeslot.time, timeslot.doctor)">
                                <img style="width: 20%;" src="../assets/time.png"> {{ timeslot.time }}
                            </button>
                        </div>
                    </div>
...
import { getAppoinmentById, GetActiveAppointmentsByDate, updateAppoinment } from '../composables/appointmentManager.js'
import { GetAllTimeslots } from '../composables/timeslotManager.js'
import { previousDate, nextDate, addDays, skipSundayAndMonday, skipMondayAndSunday, toDateString } from '../composables/datetime-utils.js'
import { combineTimeslotAppointments, findFreeTimeslots, RemoveDuplicates } from '../composables/arrayTransfromer.js'
...
.chooseDate, .chooseDoctor, .chooseTime {
    background-color: var(--lightGrey);
}
.chooseDate button, .chooseDoctor button, .chooseTime button {
    font-weight: bolder;
}
.chooseDate #box {
    width: 90px;
