    ...
    <div id="vacations-page" class="container-fluid">
        <div class="row">
            <div class="col-12">
                ...
                <div v-if="errors.genericError" class="error">{{ error.genericError }}</div>
            </div>
        </div>
            ...
            <div id="vacation-planner" class="col-11 col-lg-4">
                ...
                <form @submit.prevent="saveVacation" class="vacation-form">
                    <div class="form-group">
                        ...
                        <select v-model="newVacation.userId" class="form-select" id="employee" placeholder="Kies medewerker">
                            <option v-for="option in options" :selected="!option.value" :disabled="!option.value" :value="option.value">{{ option.text }}</option>
                        </select>
                        <div v-if="errors.employeeError" class="error">{{ errors.employeeError }}</div>
                    </div> 
                    <div class="form-group">
                        <label for="begin-datetime">Begin datum & tijd</label>
                        <input v-model="newVacation.startDateTime" type="datetime-local" class="form-control" id="begin-datetime">
                        <div v-if="errors.startDateTimeError" class="error">{{ errors.startDateTimeError }}</div>
                    </div> 
                    <div class="form-group">
                        <label for="end-datetime">Eind datum & tijd</label>
                        <input v-model="newVacation.endDateTime" type="datetime-local" class="form-control" id="end-datetime">
                        <div v-if="errors.endDateTimeError" class="error">{{ errors.endDateTimeError }}</div>
                    </div> 
                    <div class="form-group">
                        <label for="reason">Reden</label>
                        <textarea v-model="newVacation.reason" class="form-control" id="reason" rows="5"></textarea>
                        <div v-if="errors.reasonError" class="error">{{ errors.reasonError }}</div>
                    </div> 
...
export default {
    ...
    data() {
        return {
            doctors: [],
            options: [
                {value: undefined, text: 'Kies medewerker'},
            ],
            newVacation: {
                userId: undefined,
    ...
    methods: {
        ...
        validateForm() {
            ...
            if (!this.newVacation.startDateTime) {
                this.errors.startDateTimeError = 'Vergeet niet de datum en tijd in te vullen.'
                valid = false
            }
            if (!this.newVacation.endDateTime) {
                this.errors.endDateTimeError = 'Vergeet niet de datum en tijd in te vullen.'
                valid = false
            }
...
.vacation-form .form-control, .vacation-form .form-select {
    text-align: left;
    border: 3px solid var(--darkGrey);
    margin: 0;
    padding: 4px;
}
.vacation-form label, .vacation-table th {
    color: gray;
}
...
@media (max-width: 768px) {
    .vacation-form *, .vacation-table *, #vacation-overview h5 {
        font-size: 0.78rem;
    }
    #vacation-overview {
        margin-top: 24px;
